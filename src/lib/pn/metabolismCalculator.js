...
export function calculateHandPortions(user, targetCalories) {
  ...
  // Equivalencias por género según manual PN
  const portionCalories = {
    MALE: {
      P: 25,  // 1 palma masculina = ~25 calorías
      C: 25,  // 1 puño masculino = ~25 calorías  
      G: 45,  // 1 pulgar masculino = ~45 calorías
      V: 10   // 1 puño verduras = ~10 calorías
    },
    FEMALE: {
      P: 20,  // 1 palma femenina = ~20 calorías
      C: 20,  // 1 puño femenino = ~20 calorías
      G: 35,  // 1 pulgar femenino = ~35 calorías
      V: 8    // 1 puño verduras = ~8 calorías
    }
  };

  const calories = portionCalories[gender] || portionCalories.FEMALE;
  
  // Distribución de macronutrientes según PN
  // Proteínas: 25-30%, Carbohidratos: 35-45%, Grasas: 25-35%
  const proteinCalories = targetCalories * 0.275; // 27.5%
  const carbCalories = targetCalories * 0.40;     // 40%
  const fatCalories = targetCalories * 0.30;      // 30%
  const vegetableCalories = targetCalories * 0.075; // ~7.5% adicional en verduras

  // Calcular porciones
  const portions = {
    P: Math.round(proteinCalories / calories.P * 10) / 10,
    C: Math.round(carbCalories / calories.C * 10) / 10,
    G: Math.round(fatCalories / calories.G * 10) / 10,
    V: Math.round(vegetableCalories / calories.V * 10) / 10
  };
...
export function calculatePersonalizedNutrition(user) {
  try {
    ...
    // 2. Calcular TDEE
    const tdee = calculateTDEE(tmb, user.activityLevel);
    
    // 3. Ajustar según objetivo
    const targetCalories = adjustCaloriesForGoal(tdee, user.goal);
    
    // 4. Calcular porciones de mano
    const dailyPortions = calculateHandPortions(user, targetCalories);
    
    // 5. Distribuir entre comidas
